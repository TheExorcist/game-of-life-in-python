...
def update_local_copy():
	...
	content=urllib2.urlopen("https://circleci.com/api/v1/project/TheExorcist/game-of-life-in-python/?circle-token=artifact&limit=1&offset=0&filter=success").read()
	resp = map(lambda x: x.strip(),  content.split(','))
	num=(r for r in resp if r.split()[0]==':build_num').next()
